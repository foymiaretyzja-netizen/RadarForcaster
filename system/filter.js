...
const FilterSystem = {
    ...
    // Update the active filter and refresh the UI legend
    ...
    initUI: function() {
        ...
        // 1. Inject CSS for the floating panel
    ...
    drawComposite: function(ctx, cell, x, y, cellSize) {
        ...
        if (cell.clouds > 0) {
            ctx.fillStyle = `rgba(255, 255, 255, ${Math.min(0.85, cell.clouds * 0.2)})`;
            ctx.fillRect(x * cellSize, y * cellSize, cellSize, cellSize);
        }
    },
    ...
    drawFronts: function(ctx, grid, cell, i, j, cols, rows, cellSize) {
        // Calculate where the air is coming from to detect frontal boundaries
        ...
        // If the temperature shift is aggressive, paint the front
    ...
    drawWind: function(ctx, cell, x, y, cellSize) {
        // Draw small subtle lines indicating wind direction
        // We only draw every few cells to prevent canvas clutter
        if (x % 3 === 0 && y % 3 === 0) {
            ctx.beginPath();
